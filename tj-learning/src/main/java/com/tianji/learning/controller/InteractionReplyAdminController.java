...
public class InteractionReplyAdminController {
    ...
    public void hiddenReply(
            @ApiParam(value = "回答id", example = "1") @PathVariable Long id,
            @ApiParam(value = "是否隐藏，true/false", example = "true") @PathVariable Boolean hidden
    ){
    ...
    @GetMapping("{id}")
    public ReplyVO queryReplyById(@ApiParam(value = "问题id",example = "1") @PathVariable Long id){
